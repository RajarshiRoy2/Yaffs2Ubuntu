...
void SaveFiletoDesktopYaffs(const char *FilePathDesktop,const char *NameYaffs)
{
	char *filename = FilePathDesktop;
	//int SIZEOFNAME = NAME_MAX;

	char filenameBreak[NAME_MAX] ;
    strcpy(filenameBreak, FilePathDesktop);
	char *token = strtok(filenameBreak, "/");

	char filenameYaffs[NAME_MAX];

	while( token != NULL ) 
	{
	  sprintf(filenameYaffs,"%s",token);
      token = strtok(NULL, "/");
    }
	...
	sprintf(name,"%s",NameYaffs);

	char ch[1];

	h = yaffs_open(name, O_RDWR, 0);
	...
	yaffs_fstat(h,&st);//gets stats of file and store it in st
...
void SaveFileFromDesktopToYaffs(const char *FilePath,const char* filenameInYaffs)
{
	char *filename = FilePath;
	//int SIZEOFNAME = NAME_MAX;

	char filenameBreak[NAME_MAX] ;
    strcpy(filenameBreak, FilePath);
	char *token = strtok(filenameBreak, "/");

	char filenameYaffs[NAME_MAX];
	char name[NAME_MAX];

	sprintf(name,"%s",filenameInYaffs);
	...
	h = yaffs_open(name, O_CREAT | O_TRUNC | O_RDWR, 0666);//opens a file
	...
	//reads files and and prints contents to main terminal
	// printf("---------------------\n");
	//char buffer2[i];

	//h = yaffs_open(name, O_RDWR, 0);
	//yaffs_read(h,buffer2, sizeof(buffer2));
...
void AppendToFile(const char* filenameInYaffs, const char * DataToAppend)
{
	size_t length = strlen(DataToAppend);
	...
	int wrote;
	char buffer[length];
	strcpy(buffer, DataToAppend);

	h = yaffs_open(name, O_RDWR | O_CREAT | O_APPEND, 0666);//appending to a file
	wrote = yaffs_write(h, buffer, sizeof(buffer));
	yaffs_close(h);
...
int main(int argc, char *argv[])
{
	...
	start_up = yaffs_mount("/MainDrvRaj");
	file_handle = yaffs_mkdir("/MainDrvRaj/raj2",0666);
	SaveFileFromDesktopToYaffs("/home/rajarshi/Desktop/myfile.txt","/MainDrvRaj/raj2/mytext.txt");//saves myfile to mytext in yaffs2
	SaveFiletoDesktopYaffs("/home/rajarshi/Desktop/rajtxtre11.txt","/MainDrvRaj/raj2/mytext.txt");//saves mytext from yaffs2 to desktop

	char buffer[32];
	
	for(int n = 0; n<=30; n++)
	{
		sprintf(buffer,"Written to the file time %d \n",n);
		AppendToFile("/MainDrvRaj/raj2/mytext.txt",&buffer);
	}
	

	SaveFiletoDesktopYaffs("/home/rajarshi/Desktop/newfile1.txt","/MainDrvRaj/raj2/mytext.txt");

	dump_directory_tree("/MainDrvRaj");
	yaffs_unmount("/MainDrvRaj");

	
	// dump_directory_tree("/nand");
	
	// yaffs_unlink(name);//deleted file
	// dump_directory_tree("/nand");
	
	// yaffs_rmdir("/nand/raj/");//deleted folder 
